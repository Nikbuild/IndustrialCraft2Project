...
public class GeothermalGeneratorMenu extends AbstractContainerMenu {
    ...
    public final GeothermalGeneratorBlockEntity blockEntity;
    ...
    public GeothermalGeneratorMenu(int id, Inventory playerInv, GeothermalGeneratorBlockEntity blockEntity) {
        super(ModMenus.GEOTHERMAL_GENERATOR.get(), id);
        this.blockEntity = blockEntity;
        this.itemHandler = blockEntity.getInventory();

        // Add fuel slot (left, bottom) - aligned with GUI texture
        this.addSlot(new SlotItemHandler(itemHandler, GeothermalGeneratorBlockEntity.FUEL_SLOT, 65, 53));

        // Add output slot for empty cells (left, top) - aligned with GUI texture
    ...
    public GeothermalGeneratorMenu(int id, Inventory playerInv, RegistryFriendlyByteBuf buf) {
